...
1. **Explain the concepts of Reinforcement Learning (RL), Deep Learning (DL), and Federated Learning (FL) in the context of Machine Learning. Briefly describe how each approach differs in its learning mechanism, data usage, and real-world applications.** *[Combined Bank Senior Officer (IT) 17.10.2025 compact it 1427 (ET: E-Zone)]*
   ...
   - Learning mechanism — trial and error. The agent updates its policy to maximise total future reward, balancing exploration against exploitation.
   ...
   - Data usage — needs a very large labelled dataset held centrally, plus GPU or TPU power.
   - Applications — image recognition, speech to text, machine translation, medical imaging, cheque and signature verification.

   Federated Learning (FL)
   - Many devices train one shared model without ever sending their raw data to a central server.
   - Learning mechanism — the server sends the current global model to each device, each device trains on its own local data, only the weight updates are returned, and the server combines them by federated averaging into a new global model. The cycle repeats.
   - Data usage — data never leaves the device, so privacy and data-protection rules are respected.
   - Applications — mobile keyboard next-word prediction, hospital collaboration without sharing patient records, bank fraud models trained across branches without moving customer data.
   - Challenges — devices have unequal and non-identically distributed data, unreliable network connections, and communication cost.

   Key difference in one line
   - RL learns from reward, DL learns from large centralised labelled data, and FL learns from distributed data while keeping it private.
...
2. **Explain reinforcement learning in the field of Machine Learning?** *[BTCL Assistant Manager (Technical) 2023 compact it 593 (ET: BUET)]*
   ...
   - Reward (R) — the numeric feedback returned after an action.
   ...
   - The agent observes the state, takes an action, receives a reward and the next state, then updates its policy.
   - Goal — maximise total reward over time, not just the immediate reward.
   - Exploration vs exploitation — it must sometimes try new actions and other times use the best known action.
   - Two forms: positive reinforcement (reward good behaviour) and negative reinforcement (remove an unpleasant outcome).
   - Algorithms: Q-learning, SARSA, Deep Q-Network, Policy Gradient.
   - Drawback: needs many training iterations, converges slowly, and is computationally expensive.
...
3. **Weak and strong learner ensemble learning in Machine learning.** *[GTCL Assistant Engineer (CSE) 2022 compact it 686 (ET: BUET)]*
   ...
   - Usually simple and fast, such as a decision stump (a tree with a single split).
   - Has high bias but low variance.

   Strong learner
   - A model with high accuracy that generalises well to new data.
   ...
   |---|---|---|---|
   | Bagging | In parallel, on random bootstrap samples, then majority vote or average | Variance | Random Forest |
   | Boosting | Sequentially, each model correcting the previous one's errors | Bias (and variance) | AdaBoost, Gradient Boosting, XGBoost |
   | Stacking | Several different models, combined by a meta learner | Both | Stacked ensemble |

   - Why it works: individual weak learners make different mistakes, so combining them cancels much of that error out.
   - Trade-off: an ensemble is slower to train and harder to interpret than a single model.
...
1. **Write down the difference between informed and uninformed search algorithm.** *[DPDC Junior Assistant Manager (JAM) 27.06.2025 compact it 1440 (ET: BUET)]*
   ...
   | Efficiency | Slow, expands many nodes | Faster, expands far fewer nodes |
   | Time and space cost | High | Comparatively low |
   | Optimality | BFS and UCS give an optimal path | A* is optimal if `h(n)` is admissible |
   | Examples | BFS, DFS, Uniform Cost Search, Depth Limited Search, Iterative Deepening | Greedy Best First Search, A*, AO* |

   - In A*, `f(n) = g(n) + h(n)`, where `g(n)` is the cost already spent and `h(n)` is the estimated cost still to go.
   - A heuristic is admissible when it never overestimates the true remaining cost. Only then is A* guaranteed optimal.
   - Example: finding a route from Dhaka to Chittagong. BFS checks roads in every direction blindly; A* uses straight-line distance as `h(n)` and heads south-east from the start.
...
2. **How $\alpha$-$\beta$ pruning is better than minimax search in game planning?** *[ICT Ministry Assistant Programmer 2017 compact it 1243 (ET: N/A)]*
   ...
   - Beta (β) — the best score the Minimizer can guarantee so far. Starts at `+∞`.
   - When `α ≥ β` at a node, the remaining children of that node are pruned, because neither player would ever allow that line to be reached.
   - A cut at a MAX node is a beta-cutoff; a cut at a MIN node is an alpha-cutoff.

   Why it is better than plain minimax
   - Minimax must evaluate every node of the game tree; alpha-beta skips large parts of it.
   - Time complexity drops from `O(b^d)` to `O(b^(d/2))` in the best case, where `b` is the branching factor and `d` the depth.
   - That effectively doubles the search depth reachable in the same time, so the engine looks further ahead and plays stronger.
   - Fewer nodes are stored, so memory use also falls.
   - Accuracy is not lost — the chosen move is identical to plain minimax, since only branches that cannot affect the decision are removed.

   - Its gain depends on move ordering. If the best move is examined first, pruning is maximum; with the worst ordering it degrades to plain minimax. Engines therefore use heuristics to try promising moves first.
...
1. **In machine learning. What will happen, when a machine is highly trained up a slight trained up?** *[BPDB Assistant Engineer (CSE) 10.05.2024 compact it 395 (ET: BUET)]*
   ...
   - Training accuracy is very high but test accuracy is low, leaving a wide gap between them.
   - It has low bias and high variance — the model is too flexible and reacts to every detail.
   - Causes: too complex a model, too many features, too little training data, no regularization.
   - Fixes: collect more data, simplify the model, apply L1/L2 regularization, use dropout in neural networks, prune decision trees, stop training early, and clean noisy data.
   ...
   - Causes: too simple a model, too much regularization, weak or missing features, too few training epochs.
   - Fixes: use a more complex model, add better features, reduce regularization, train longer, scale the features properly.

   Good fit
   - The target is the balance in the middle — complex enough to capture the real pattern, but not so complex that it memorises noise.
   - Training error and validation error are both low and close to each other.
