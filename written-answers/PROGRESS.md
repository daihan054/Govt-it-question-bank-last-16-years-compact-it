# Answer writing progress

`written-answers/` — model answers for every question in `written/`.
Commit unit: one `##` subtopic.

**Overall: 239/239 subtopics · 2614/2673 answers** — 59 questions skipped as content-free or incomplete (figure, code or question text not reproduced in the source)

## ai-and-ml.md — 11/11 sections, 41/41 answers

- [x] Artificial Intelligence & Expert Systems (8)
- [x] Deep Learning & Neural Networks (ANN, CNN, RNN) (7)
- [x] Machine Learning Paradigms (Supervised vs Unsupervised) (6)
- [x] Model Evaluation & Datasets (5)
- [x] Supervised Learning (Decision Trees) (4)
- [x] Generative AI & Explainable AI (XAI) (4)
- [x] Advanced Machine Learning & Deep Learning (RL, DL, Federated Learning) (3)
- [x] Search Algorithms (Informed vs Uninformed Search) (1)
- [x] Overfitting, Underfitting & Model Generalization (1)
- [x] Association Rule Learning (Market Basket Analysis) (1)
- [x] Clustering & Unsupervised Learning (K-Means, Hierarchical) (1)

## algorithm.md — 14/14 sections, 113/113 answers

- [x] Sorting Algorithms & Complexity (27)
- [x] Graph Traversal Algorithms (BFS & DFS) (17)
- [x] Graph Algorithms (Shortest Path & Minimum Spanning Tree) (14)
- [x] Algorithm Analysis & Asymptotic Complexity (12)
- [x] Searching Algorithms (11)
- [x] Dynamic Programming & Greedy Algorithms (7)
- [x] Graph Theory & Isomorphism (7)
- [x] Greedy Algorithms (Fractional Knapsack) (6)
- [x] Dynamic Programming (5)
- [x] Heap & Priority Queue (2)
- [x] Graph Representation (Adjacency Matrix vs List) (2)
- [x] Divide and Conquer & Matrix Multiplication (1)
- [x] Huffman Coding & Data Compression (1)
- [x] NP-Completeness & Complexity Reduction (1)

## bangla.md — 6/6 sections, 123/123 answers

- [x] বাংলা ব্যাকরণ ও সাহিত্য (61)
- [x] Focus Writing (25)
- [x] Translation (19)
- [x] পত্র লিখন (7)
- [x] সারমর্ম / সারাংশ (6)
- [x] এক কথায় প্রকাশ (One Word Substitution) (5)

## c-programming.md — 9/9 sections, 202/202 answers

- [x] Basic Programs & Control Statements (86)
- [x] Output Tracing & Control Flow (35)
- [x] Recursion & Functions (32)
- [x] Operators, Data Types & Language Concepts (17)
- [x] Flowcharts & Algorithms (12)
- [x] String Manipulation & Algorithms (11)
- [x] File Handling (4)
- [x] Pointers (4)
- [x] Command Line Arguments & Basic Programs (1)

## cloud-computing.md — 9/9 sections, 34/34 answers

- [x] Cloud Service Models (12)
- [x] Cloud Storage & Fundamentals (6)
- [x] Virtualization & Containers (VM vs Container) (6)
- [x] Cluster, Grid & Distributed Computing (3)
- [x] Scalability (Horizontal & Vertical Scaling) (2)
- [x] Edge Computing & Fog Computing (2)
- [x] Virtualization & Resource Allocation (1)
- [x] High Availability & System Redundancy (1)
- [x] Cloud Security & Compliance (1)

## compiler-and-toc.md — 5/5 sections, 20/20 answers

- [x] Regular Expressions & Finite Automata (6)
- [x] Grammar & Ambiguity (5)
- [x] Compiler vs Interpreter (4)
- [x] Lexical Analysis & Compiler Phases (4)
- [x] Linker & Loader (1)

## computer-fundamental.md — 10/10 sections, 127/127 answers

- [x] Computer Fundamentals & Acronyms (49)
- [x] Hardware Components & BIOS (CMOS Battery) (21)
- [x] ICT in Society & Governance (20)
- [x] Software Types & Classification (12)
- [x] Blockchain & Emerging Technologies (7)
- [x] Data Center Infrastructure & Power Management (7)
- [x] Server Hardware & Enterprise Systems (5)
- [x] Quantum Computing & Emerging Technologies (3)
- [x] Digital Banking & Financial Inclusion (2)
- [x] User Interfaces (CLI vs GUI) (1)

## computer-network-security.md — 14/14 sections, 156/156 answers

- [x] Cryptography (27)
- [x] Social Engineering & Cyber Attacks (26)
- [x] Firewalls & Network Defense (16)
- [x] Authentication & Access Control (15)
- [x] Web Security Vulnerabilities (15)
- [x] Malware & Security Threats (15)
- [x] Security Protocols (SSL/TLS, HTTPS) (11)
- [x] Cyber Crime & Security (9)
- [x] Security Principles (CIA Triad) (7)
- [x] VPN & Tunneling Protocols (IPsec, SSL VPN) (6)
- [x] Critical Information Infrastructure (CII) & Cyber Governance (3)
- [x] Cryptography & Network Security Scenarios (3)
- [x] Email & Messaging Security (Spam, Phishing) (2)
- [x] Buffer Overflow & Software Vulnerabilities (1)

## computer-networks.md — 33/33 sections, 440/440 answers

- [x] Subnetting & IP Addressing (95)
- [x] OSI & TCP/IP Reference Model (43)
- [x] Networking Fundamentals & Terminology (23)
- [x] Application Layer Protocols & Troubleshooting (DNS, DHCP, HTTPS) (19)
- [x] Multiplexing & Bandwidth (18)
- [x] Routing Protocols & Route Configuration (18)
- [x] Wireless Networks & IoT (mmWave) (17)
- [x] Transport Layer (TCP & UDP) (15)
- [x] Networking Devices (14)
- [x] Communication System & Transmission Modes (14)
- [x] Physical Layer & Transmission Media (Cables & Wiring) (14)
- [x] Error Detection & Data Communication (CRC, Throughput) (14)
- [x] Data Rate & Channel Capacity (Nyquist, Shannon) (14)
- [x] Network Topologies (12)
- [x] IPv6 Addressing (11)
- [x] Physical Layer & Optical Fiber (Attenuation & Power Budget) (11)
- [x] Network Address Translation (NAT) (11)
- [x] Network Services (DHCP, NAT) (10)
- [x] Digital Modulation & Signal Processing (BPSK, QPSK) (10)
- [x] Flow Control & Data Link Layer (Stop-and-Wait) (9)
- [x] Email Architecture & Protocols (SMTP, POP3, IMAP) (9)
- [x] Application Layer & Well-Known Port Numbers (6)
- [x] Switching Techniques (Circuit vs Packet Switching) (5)
- [x] WAN Technologies (SONET/SDH, ATM, WDM) (5)
- [x] Pulse Code Modulation (PCM) & Signal Processing (4)
- [x] Network Layer (Packet Fragmentation & Tunneling) (4)
- [x] Analog Modulation & Radio Receivers (3)
- [x] Satellite Communication (3)
- [x] Line Coding & Digital Encoding (2)
- [x] Address Resolution (ARP & RARP) (2)
- [x] VLANs & Subnetting Comparison (2)
- [x] Spread Spectrum & Multiple Access (CDMA, FHSS, DSSS) (2)
- [x] High Availability & Redundancy Protocols (VRRP, HSRP) (1)

## data-structure.md — 8/8 sections, 86/86 answers

- [x] Tree (26)
- [x] Stack (19)
- [x] Linked List (14)
- [x] Priority Queues & Heaps (Min/Max Heap) (7)
- [x] Queue (6)
- [x] Binary Search Tree (BST) (6)
- [x] Hashing & Hash Tables (6)
- [x] Data Structure Fundamentals (2)

## database.md — 19/19 sections, 229/229 answers

- [x] SQL Queries (71)
- [x] DBMS Architecture & Features (22)
- [x] ER Diagram & Database Design (21)
- [x] Keys in DBMS (21)
- [x] Normalization & Database Design (18)
- [x] SQL Commands (DDL, DML, DCL, TCL) (13)
- [x] Transaction Management & ACID Properties (12)
- [x] Relational Data Model & ER Relationships (11)
- [x] Database Backup & Disaster Recovery (8)
- [x] PL/SQL & Database Triggers (6)
- [x] Indexing & Query Optimization (B-Tree, B+ Tree) (6)
- [x] Distributed & Parallel Databases (4)
- [x] Data Warehousing, Data Mining & Business Intelligence (4)
- [x] Database Design & Data Types (3)
- [x] SQL Joins & Operations (3)
- [x] NoSQL, NewSQL & Modern Databases (2)
- [x] Database Connectivity (JDBC) (2)
- [x] Relational Keys (Candidate, Super, Primary, Foreign Key) (1)
- [x] Indexing in DBMS (1)

## dld.md — 9/9 sections, 110/110 answers

- [x] Logic Gates & Universal Gates (27)
- [x] Number Systems & Base Conversions (19)
- [x] Combinational Circuits (Adders, Encoders, MUX) (18)
- [x] Karnaugh Map (K-Map) (16)
- [x] Boolean Algebra & De Morgan’s Theorem (13)
- [x] Sequential Circuits (Latches & Flip-Flops) (9)
- [x] Logic Families (TTL vs CMOS) (5)
- [x] 2's Complement & Binary Arithmetic (2)
- [x] Finite State Machines (FSM) (1)

## electrical-and-electronics.md — 8/8 sections, 37/37 answers

- [x] Electrical Circuits & Protection Devices (12)
- [x] Transistors (BJT & FET) (9)
- [x] Semiconductor Devices & Diodes (4)
- [x] Digital-to-Analog & Analog-to-Digital Converters (DAC/ADC) (4)
- [x] AC Circuits & Power Analysis (2)
- [x] Operational Amplifiers (Op-Amp) (2)
- [x] Sensor Circuits & Automated Control Systems (2)
- [x] Circuit Theorems (Thevenin, Norton, Superposition) (2)

## english.md — 8/8 sections, 110/110 answers

- [x] Focus Writing (35)
- [x] English Grammar (29)
- [x] Translation (18)
- [x] Idioms & Phrases (9)
- [x] English Vocabulary & Antonyms (7)
- [x] Letter & Application Writing (6)
- [x] Reading Comprehension (5)
- [x] English Literature & Authors (1)

## gk.md — 4/4 sections, 214/214 answers

- [x] Bangladesh Affairs (114)
- [x] International Affairs (70)
- [x] Everyday Science & Environment (22)
- [x] Banking & ICT Abbreviations (8)

## image-processing.md — 5/5 sections, 7/7 answers

- [x] Computer Graphics (Vector & Raster) (3)
- [x] Color Models (1)
- [x] Frequency Domain Filtering (1)
- [x] Edge Detection (1)
- [x] Morphological Operations (1)

## math.md — 15/15 sections, 88/88 answers

- [x] Arithmetic & Algebra Problems (16)
- [x] Set Theory & Discrete Math (13)
- [x] Percentage, Profit & Loss, Simple & Compound Interest (12)
- [x] Basic Arithmetic & Average (11)
- [x] Geometry & Coordinate Geometry (10)
- [x] Permutations & Combinations (5)
- [x] Ratio, Proportion & Mixtures (4)
- [x] Speed, Time, Distance & Boats (4)
- [x] Probability & Statistics (3)
- [x] Calculus & Integration (2)
- [x] Analytical Ability & Logical Reasoning (2)
- [x] Comprehensive Math Problems (2)
- [x] Discrete Mathematics & Recurrence Relations (2)
- [x] Propositional Logic & Logical Equivalence (1)
- [x] Numerical Methods & Root Finding (1)

## microprocessor-and-computer-architecture.md — 11/11 sections, 107/107 answers

- [x] Microprocessor Architecture & Functions (27)
- [x] Memory Hierarchy & Storage (21)
- [x] RAID Architecture & Storage (13)
- [x] Cache Memory (12)
- [x] Secondary Storage (HDD vs SSD) (10)
- [x] Multi-Core & Multi-Threading (5)
- [x] Assembly Language & Addressing Modes (5)
- [x] Instruction Pipelining & Hazards (5)
- [x] CPU Performance & Instruction Cycle (4)
- [x] 8085 Microprocessor & Edge Computing (3)
- [x] RISC vs CISC Architecture (2)

## oop.md — 9/9 sections, 84/84 answers

- [x] OOP Concepts (Inheritance & Polymorphism) (45)
- [x] Java Programming & Methods (11)
- [x] Class Design & Object-Oriented Modeling (7)
- [x] Encapsulation & Access Modifiers (6)
- [x] Constructors & Destructors (5)
- [x] Output Tracing & Recursion (3)
- [x] Exception Handling (3)
- [x] C++ OOP Concepts & Friend Functions (2)
- [x] Interfaces & Abstract Classes (2)

## operating-system.md — 12/12 sections, 168/168 answers

- [x] Linux / Unix Commands & Administration (42)
- [x] CPU Scheduling Algorithms (24)
- [x] Deadlock & Resource Allocation (22)
- [x] OS Concepts & System Software (15)
- [x] Virtual Memory & Page Replacement (Thrashing) (15)
- [x] Memory Management & Paging (13)
- [x] Process Management & Process States (10)
- [x] Concurrency, Threads & Synchronization (9)
- [x] CPU Scheduling (6)
- [x] Windows & System Administration (4)
- [x] Process Synchronization & Concurrency (4)
- [x] File Systems & Disk Management (4)

## software-engineering.md — 13/13 sections, 124/124 answers

- [x] SDLC Phases & Models (37)
- [x] Software Testing & Evaluation (33)
- [x] Software Architecture & Design Patterns (MVC) (11)
- [x] UML Diagrams (Class, Use Case, Sequence) (9)
- [x] Software Requirements Engineering (8)
- [x] Software Project Management & Organization (7)
- [x] Software Design Principles (Coupling & Cohesion) (5)
- [x] Software Cost Estimation & Build vs Buy Decisions (4)
- [x] IT Governance, Audit & Risk Management (3)
- [x] Data Flow Diagrams (DFD) (2)
- [x] Code Smells & Refactoring (2)
- [x] Open Source Software & Licensing (2)
- [x] CI/CD & DevOps Methodologies (1)

## web-technology.md — 7/7 sections, 53/53 answers

- [x] HTML & Web Fundamentals (17)
- [x] HTTP Protocol (10)
- [x] JavaScript & jQuery (DOM & Validation) (8)
- [x] Web Services & APIs (SOAP vs REST) (7)
- [x] Full Stack & Backend Web Development (5)
- [x] CSS & Styling (Inline, Internal, External) (4)
- [x] Web Security & Browser Same-Origin Policy (Iframe) (2)
