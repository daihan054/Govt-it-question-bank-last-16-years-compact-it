...
Answer:
   ...
   | DHCP | Dynamic Host Configuration Protocol | Assigns IP addresses automatically, ports 67/68 |
...
Answer: A register is a very small, very fast storage location inside the CPU. It holds the data, instruction or address the processor is working on at that instant. Registers are the fastest memory in the hierarchy — faster than cache and RAM.
   ...
   | MAR | Memory Address Register | Holds the memory address to be read from or written to |
   | MDR / MBR | Memory Data (Buffer) Register | Holds the data being transferred to or from memory |
   | AC | Accumulator | Holds intermediate results of ALU operations |
   | SP | Stack Pointer | Points to the top of the stack |
   | Flag / PSW | Status register | Holds condition flags — zero, carry, sign, overflow |
   | General purpose | R0-Rn, AX, BX in x86 | Hold operands and temporary data |

   - Registers are measured in words, not bytes — a 64-bit CPU has 64-bit registers.
   - They are few in number because they are built from expensive, fast flip-flops directly on the CPU die.
...
Answer: **1 TB = 2⁴⁰ bytes = 1,099,511,627,776 bytes** (binary measure, about 1.1 trillion bytes).
   ...
   - Full ladder: `1 KB = 1024 B`, `1 MB = 1024 KB`, `1 GB = 1024 MB`, `1 TB = 1024 GB`, `1 PB = 1024 TB`.
...
Answer: The base of the fifth generation computer is **Artificial Intelligence (AI)**, built on **ULSI (Ultra Large Scale Integration)** technology.
    ...
    - Languages used: LISP and PROLOG for AI work.
    - Examples: modern laptops, desktops, IBM Watson, robots.
...
Answer: A computer generation means a stage of development in computer technology, marked by a major change in the ELECTRONIC COMPONENT used to build the processor. Each generation is faster, smaller, cheaper and more reliable than the one before it.

    | Generation | Period | Main component | Language | Example |
    |---|---|---|---|---|
    | First | 1940-1956 | Vacuum tube | Machine language | ENIAC, UNIVAC, EDVAC |
    | Second | 1956-1963 | Transistor | Assembly, COBOL, FORTRAN | IBM 1401, IBM 7094 |
    | Third | 1964-1971 | Integrated Circuit (IC) | High-level languages, OS appears | IBM 360, PDP-8 |
    | Fourth | 1971-1980 | Microprocessor (VLSI) | C, C++, GUI, networks | IBM PC, Apple II |
...
| LRU |  |
    ...
    |---|---|---|
    | GUI | Graphical User Interface | A visual interface using windows, icons, menus and a pointer, so the user clicks instead of typing commands |
    | RAID | Redundant Array of Independent Disks | Combines several physical disks into one logical unit for speed (striping), redundancy (mirroring) or both |
    | API | Application Programming Interface | A defined set of functions and rules that lets one program request services from another |
    | LRU | Least Recently Used | A page or cache replacement policy that evicts the item unused for the longest time |

    Quick extra detail
    - RAID levels: RAID 0 striping (speed, no redundancy), RAID 1 mirroring, RAID 5 striping with distributed parity, RAID 10 mirror plus stripe.
    - LRU is used in operating system page replacement and in CPU cache eviction; alternatives are FIFO and Optimal.
...
Answer: **OCR (Optical Character Recognition)** is the technology that reads printed or handwritten TEXT from a scanned image and converts it into editable, machine-readable text.
    ...
    | What it reads | Characters — letters, digits, words | Marks — filled bubbles, ticks in fixed positions |
    | Output | Editable text | Position of the mark, mapped to an answer |
    | Complexity | Complex, needs pattern recognition and often AI | Simple, only detects light and dark areas |
    | Accuracy | Lower, affected by font and handwriting | Very high, since only presence of a mark matters |
...
Answer:
    ...
    - The main working memory of a computer. The CPU reads programs and data from it while running.
    - Volatile — everything is lost when power is switched off.
    - Two types: SRAM (fast, used for cache) and DRAM (slower, cheaper, used as main memory).

    (ii) ROM — Read Only Memory
    - Non-volatile memory whose contents survive power loss.
    - Holds firmware such as the BIOS that starts the machine.
    - Types: PROM (write once), EPROM (erased by UV light), EEPROM and flash (erased electrically).

    (iii) Primary key
    - A column, or set of columns, that uniquely identifies each row of a table.
    - Cannot be NULL and cannot repeat. A table has exactly one primary key.
    - Example: `student_id` in a Student table.

    (iv) Foreign key
    - A column in one table that refers to the primary key of another table, creating a link between them.
    - Enforces referential integrity — a value must exist in the parent table.
    - May be NULL and may repeat. Example: `dept_id` in an Employee table pointing to Department.

    (v) Data
    - Raw, unprocessed facts and figures with no meaning on their own, such as `45`, `Rahim`, `2024`.
    - When data is processed and given context it becomes information — "Rahim scored 45 marks in 2024".
...
Answer:
    ...
    |---|---|---|
    | Meaning | The physical machine that processes data | The complete arrangement of hardware, software, data, people and procedures working together |
    | Scope | Narrow — the device itself | Broad — the device plus everything needed to use it |
    | Components | CPU, memory, input and output devices | Hardware + Software + Data + People (liveware) + Procedures |
    | Can it work alone | No — hardware without software does nothing | Yes — it is a complete working unit |
...
Answer: As of the TOP500 list of June 2026, the world's most powerful supercomputer is **LineShine**, installed at the National Supercomputing Centre in Shenzhen, China.
    ...
    | Rank | System | Country | Performance |
    |---|---|---|---|
    | 1 | LineShine | China | 2.198 EFlop/s |
    | 2 | El Capitan | USA | 1.809 EFlop/s |
    | 3 | Frontier | USA | about 1.35 EFlop/s |

    - Note: this ranking changes twice a year, at the ISC conference in June and SC in November, so the answer must be checked against the current TOP500 list.
...
Answer:
    ...
    |---|---|---|
    | Meaning | Raw, unorganised facts and figures | Processed data that carries meaning |
    | Dependency | Does not depend on information | Depends on data |
    | Usefulness | Not useful by itself | Useful for decision making |
    | Form | Numbers, characters, symbols, images | Reports, tables, charts, summaries |
...
Answer: Unicode defines **1,114,112 code points** in total.
    ...
    - Plane 0 is the Basic Multilingual Plane (BMP), holding almost all commonly used characters including Bangla, English, Arabic and Chinese.
    - Of these code points, roughly 150,000 are actually assigned to characters so far; the rest are reserved for future use.
    - Encoding forms: UTF-8 (1-4 bytes, most used on the web), UTF-16 (2 or 4 bytes) and UTF-32 (fixed 4 bytes).
    - A common exam answer is "16-bit Unicode gives 65,536 characters" — that refers only to the original UCS-2 / BMP, not to Unicode as it exists today.
...
Answer: **(b) 16 bits.**
    ...
    - But in exam multiple-choice questions the expected answer remains 16 bits, contrasted with ASCII, which uses 7 bits (8 with the parity bit) for 128 characters.
...
Answer: **(b) Global Positioning System.**
    ...
    - Other global systems: GLONASS (Russia), Galileo (Europe), BeiDou (China), NavIC (India).
...
Answer: **The fourth generation (1971-1980).**
    ...
    | Generation | Component | Transistors per chip |
    |---|---|---|
    | Third | SSI / MSI Integrated Circuit | tens to hundreds |
    | Fourth | VLSI | thousands to hundreds of thousands |
    | Fifth | ULSI | millions and above |
...
Answer:
    ...
    | Acronym | Full form | Function | Port |
    |---|---|---|---|
    ...
    | FTP | File Transfer Protocol | Transfers files between client and server | 20 (data), 21 (control) |
    | ARP | Address Resolution Protocol | Finds the MAC address for a known IP address | — |
    | UDP | User Datagram Protocol | Connectionless, fast, unreliable delivery | — |

    - TCP and UDP are both transport-layer protocols. TCP is used for web, email and file transfer; UDP for DNS, video streaming, VoIP and online games, where speed matters more than reliability.
    - ARP works at the boundary of layers 2 and 3, mapping a layer-3 IP address to a layer-2 MAC address.
...
Answer: By size, capacity and processing power, digital computers are divided into four types.

    | Type | Size | Users | Speed / capacity | Example use |
    |---|---|---|---|---|
    | Supercomputer | Fills a large room | Many, through jobs | Fastest — petaflops to exaflops | Weather forecasting, nuclear simulation, genome research |
    | Mainframe | Large cabinets | Thousands simultaneously | Very high throughput, huge storage | Bank core systems, airline reservation, census |
    | Minicomputer | Cabinet or rack size | Tens to hundreds | Medium | Departmental servers, process control in factories |
    | Microcomputer | Desktop or smaller | One at a time | Lowest, but sufficient for personal work | PC, laptop, tablet, smartphone |

    (a) Supercomputer
    - The most powerful class, used for problems needing enormous calculation.
    - Measured in FLOPS. The current leader, LineShine in China, runs at about 2.2 exaflops.
    - Bangladesh's BMD supercomputer is used for weather prediction.

    (b) Mainframe computer
    - Built for very high RELIABILITY and TRANSACTION VOLUME rather than raw calculation speed.
    - Supports thousands of simultaneous users through terminals.
    - Used by banks, insurance companies, railways and government record systems. Example: IBM Z series.

    (c) Minicomputer (midrange)
    - Sits between mainframe and microcomputer. Serves a department rather than a whole organisation.
    - Largely replaced today by powerful servers.

    (d) Microcomputer
    - Built around a single microprocessor. Includes desktop, laptop, tablet, smartphone and embedded systems.
    - The most widely used class by far.

    - A workstation is a high-end microcomputer built for engineering, CAD and graphics work.
...
a) What is full name of ASCII?
   ...
   | (g) | Refresh rate |
   | (h) | 254 |
   | (i) | 4 bits (half a byte) |
   | (j) | 7 layers |
   | (k) | Video Graphics Array |
   | (l) | 16 bits |
   ...
   Notes
   - (g) Refresh rate is measured in hertz (Hz) — 60 Hz means the screen redraws 60 times per second. Resolution is in pixels and size in inches.
   - (j) OSI layers from bottom: Physical, Data Link, Network, Transport, Session, Presentation, Application.
...
Answer:
    ...
    **BIOS — Basic Input Output System**
    ...
    - Modern machines use UEFI instead, which supports disks larger than 2 TB, faster boot and Secure Boot.
...
Answer:
    ...
    - Using the binary standard where `1 MB = 1024 × 1024 = 1,048,576 bytes`:
...
60. **(c) Compare TTL and CMOS logic family in terms of (i) Speed (ii) Noise and (iii) Power consumption.** *[Bangladesh Public Service Commission Ministry of Power, Energy and Mineral Resources Assistant Maintenance Engineer; Date: 30 May, 2025 Exam Taker: BPSC; Written [bitbox it book 69]]*
    ...
    | Parameter | TTL (Transistor-Transistor Logic) | CMOS (Complementary Metal-Oxide-Semiconductor) |
    |---|---|---|
    | Component Technology | Bipolar Junction Transistors (BJTs) | Complementary pairs of pMOS and nMOS transistors |
    | (i) Switching Speed | High propagation speed; typical delay is 10 ns (standard TTL) to 3 ns (Schottky) | Historically slower, but modern CMOS (74ACT) matches TTL speeds |
    | (ii) Noise Margin | Moderate noise immunity (approx. 0.4 V noise margin) | Excellent noise immunity (approx. 30% to 45% of VDD, 1.5 V) |
...
66. **A CPU has 512 pages of each size 2 KB, and total frames = 128. Determine the length of logical address and physical address.** *[Bangladesh Bridge Authority Post: Assistant Programmer; Date: 12 July, 2025 Exam Taker: IBA; Written: 80 Marks Tech: 3*10=30, Non-Tech: Bangla 10, Math 10, English 15, GK 15 [bitbox it book 92]]*
    ...
    - Page size = $2	ext{ KB} = 2 	imes 1024	ext{ bytes} = 2048	ext{ bytes} = 2^{11}	ext{ bytes}$
    - Number of frames = $128 = 2^7$

    1. Logical Address Calculation:
    - Page Number bits ($p$) = $\log_2(512) = 9	ext{ bits}$
    - Offset / Page Size bits ($d$) = $\log_2(2048) = 11	ext{ bits}$
    - Total Logical Address Length = $p + d = 9 + 11 = 20	ext{ bits}$

    2. Physical Address Calculation:
    - Frame Number bits ($f$) = $\log_2(128) = 7	ext{ bits}$
...
85. **Draw State Diagram for an Online Shopping Website. (05)** *[বাংলাদেশ পল্লী বিদ্যুতায়ন বোর্ড (BREB) তারিখ: ২১/১২/২০২৫ পূর্ণমান: ১০০ সময়: ২.০০ ঘণ্টা পদের নাম: সহকারী প্রোগ্রামার [bitbox it book 313]]*
    ...
    stateDiagram-v2
        [*] --> Browsing
        Browsing --> Cart: Add Item to Cart
        Cart --> Checkout: Proceed to Checkout
        Checkout --> PaymentPending: Select Payment Method
        PaymentPending --> OrderConfirmed: Payment Successful
        PaymentPending --> PaymentFailed: Payment Declined
        PaymentFailed --> Checkout: Retry Payment
        OrderConfirmed --> Processing: Order Dispatched
        Processing --> Delivered: Item Delivered
        Delivered --> [*]
    ```
...
89. **(a) Write a short comparative note on “Static web page” and “Dynamic web page”. [4 marks]** *[Bangladesh Public Service Commission Assistant Maintenance Engineer; Date: 09 February, 2024 Exam Taker: BPSC; Written [bitbox it book 332]]*
    ...
    |---|---|---|
    | Content Generation | Pre-built content delivered exactly as stored on server | Generated in real-time based on user request and database queries |
    | Technologies Used | HTML, CSS, client-side JavaScript | PHP, Node.js, Python, Java, coupled with SQL/NoSQL databases |
    | Database Dependency | Does not use a database | Interacts with databases to fetch dynamic content |
    | Loading Speed | Extremely fast due to static caching | Slightly slower due to server-side script execution and query time |
...
90. **(b) What is malware? State in short with some examples? [5 marks]** *[Bangladesh Public Service Commission Assistant Maintenance Engineer; Date: 09 February, 2024 Exam Taker: BPSC; Written [bitbox it book 332]]*
    ...
    - Virus: Attaches to legitimate executable files and replicates when the host program runs (e.g., CIH, ILOVEYOU).
    - Worm: Self-replicating standalone malware that spreads across networks without human intervention (e.g., Stuxnet, SQL Slammer).
    - Trojan Horse: Disguises itself as legitimate software to create backdoors for attackers (e.g., Zeus Trojan).
...
91. **(c) You have been given the responsibility of identifying software requirements from a customer. Which methods would you follow to gather those requirements? [5 marks]** *[Bangladesh Public Service Commission Assistant Maintenance Engineer; Date: 09 February, 2024 Exam Taker: BPSC; Written [bitbox it book 333]]*
    ...
    - 2. Surveys and Questionnaires: Distributing targeted questionnaires to collect quantitative requirements from a large user base.
    - 3. Joint Application Development (JAD) Workshops: Facilitating collaborative intensive brainstorming sessions with users, developers, and management.
    - 4. Prototyping: Developing interactive mockups and wireframes to help users visualize functionalities and refine expectations.
...
92. **What are the various types of relationships in Database? Define them.** *[Bankers' Selection Committee Secretariat Post: Assistant Programmer; Date: 15 Feb, 2024 Exam Taker: ANZA; Post: 35 [bitbox it book 355]]*
    ...
    Three Primary Types:
    - 1. One-to-One (1:1):
      - A record in Table A is linked to at most one record in Table B, and vice versa.
      - Example: `User` and `PassportDetails` (One citizen has exactly one passport).
    - 2. One-to-Many (1:N):
      - A record in Table A can be associated with multiple records in Table B, but a record in Table B relates to only one record in Table A.
      - Example: `Department` and `Employee` (One department has many employees, each employee belongs to one department).
    - 3. Many-to-Many (M:N):
    ...
    def calculate_tax(income):
        ...
        if income <= 350000:
            ...
            tax = (100000 * 0.05 + 300000 * 0.10 + 400000 * 0.15 + 
                   500000 * 0.20 + (income - 1650000) * 0.25)
        return tax
...
99. **Question: Matching with Definition** *[Jamuna Oil Company Ltd Post: Junior Officer (MIS & IT); Date: 23 May, 2024 Exam Taker: JOCL [compact it 437]]*
    ...
    - ROM: Read-Only Memory (Non-volatile memory holding boot firmware/BIOS).
...
102. **Draw diagrams of Bus, Star, and Mesh network topologies.** *[Jamuna Oil Company Ltd Post: Junior Officer (MIS & IT); Date: 23 May, 2024 Exam Taker: JOCL [compact it 438]]*
    ...
    1. Bus Topology (Linear Backbone):
    ```
    ...
    2. Star Topology (Central Switch/Hub):
    ```
              Node 1
                |
       Node 4-[HUB/SWITCH]-Node 2
                |
              Node 3
    ```
...
103. **Write the names of three linear and three non-linear data structures. Also, mention which type of data structure a Binary Search Tree (BST) is.** *[Jamuna Oil Company Ltd Post: Junior Officer (MIS & IT); Date: 23 May, 2024 Exam Taker: JOCL [compact it 439]]*
    ...
    - Three Linear Data Structures:
      1. Array
      2. Stack
      3. Queue (or Linked List)
    - Three Non-Linear Data Structures:
...
112. **Differentiate among TDM, FDM, and WDM. How does synchronous TDM work?** *[Bankers' Selection Committee Secretariat Post: Senior Office (IT); Date: 04 October, 2024 Exam Taker: ANZA; Post: 222 [bitbox it book 514]]*
    ...
    | Feature | FDM (Frequency Division Multiplexing) | TDM (Time Division Multiplexing) | WDM (Wavelength Division Multiplexing) |
    |---|---|---|---|
    | Signal Type | Analog signals | Digital signals | Optical (Light) signals |
    | Division Basis | Total channel bandwidth divided into frequency slots | Total time divided into discrete time slots | Optical fiber spectrum divided into distinct wavelengths (colors) |
    | Guard Bands | Guard bands needed to prevent channel overlap | Guard intervals/times used to prevent slot collision | Guard bands in optical frequency spectrum |
    | Medium | Coaxial cable, radio, wireless | Digital transmission links (T1/E1 lines) | Fiber-optic cables |

    How Synchronous TDM Works:
    - In Synchronous TDM, time is divided into equal, fixed-duration time slots.
...
Answer: The CMOS battery is a small coin cell (usually CR2032, 3 V) on the motherboard. Its job is to keep the CMOS chip powered when the computer is switched off, so that BIOS settings and the real-time clock survive.

   What the CMOS battery maintains
   - System date and time.
   - BIOS/UEFI configuration — boot order, hardware settings, enabled or disabled ports.
   ...
   - The clock resets to a default date such as 01/01/2000 every time the machine is switched off.
   - BIOS settings, including boot order, revert to defaults on every restart.
   - Error messages at boot: "CMOS checksum error", "CMOS battery failure", "System time not set".
   - The machine may refuse to boot from the intended device because the boot order was lost.
   - SSL certificate errors while browsing, caused by the wrong system date.

   - Replacement is simple and cheap: power off, unplug, remove the coin cell, insert a new CR2032 and reconfigure BIOS settings. Note that removing the battery is also the standard way to reset a forgotten BIOS password.
...
Answer:
   ...
   - BIOS is firmware stored in a ROM/flash chip on the motherboard. It is the FIRST code the CPU executes when power is applied.
   - It initialises and tests the hardware, provides basic input/output routines, reads its configuration from CMOS, then finds the boot device and hands control to the operating system's bootloader.

   Steps from power-on to OS loading

   ```mermaid
   flowchart TD
       A[Power switch pressed] --> B[PSU sends 'Power Good' signal]
       B --> C[CPU jumps to the reset vector<br/>and starts executing BIOS]
       C --> D[POST — Power-On Self-Test]
       D --> E{Hardware OK?}
       E -->|No| F[Beep codes / error message, stop]
       E -->|Yes| G[Initialise devices, read CMOS settings]
       G --> H[Find the boot device in the boot order]
       H --> I[Load the MBR from sector 0 into RAM]
       I --> J[MBR runs the bootloader — GRUB / Windows Boot Manager]
       J --> K[Bootloader loads the OS kernel into RAM]
...
Answer:
   ...
   |---|---|---|
   | Data transmission | One bit at a time, over a single wire | 8 bits at a time, over 8 separate wires |
   | Connector | 9-pin (DB-9) or 25-pin (DB-25) | 25-pin (DB-25) female |
   | Speed over short distance | Slower per clock, but higher clock rates possible | Faster at the same clock rate |
   | Distance supported | Long — up to 50 feet or more | Short — about 15 feet before signals degrade |
   | Cable cost | Cheaper, fewer wires | Costlier, more wires |
   | Crosstalk / skew | Minimal | Significant — bits can arrive at different times on long cables |
   | Typical devices | Modem, mouse, network router console, industrial sensors | Printer, scanner, external ZIP drive |
   | Also called | COM port | LPT port (Line Print Terminal) |

   - The classic assumption "parallel is faster" holds only over short distances. At high frequencies, the 8 wires suffer from skew and crosstalk, which is exactly why modern high-speed interfaces — USB, SATA, PCIe — all went back to SERIAL transmission.
   - Both ports are now obsolete on consumer machines and have been replaced by USB.
...
Answer:
   ...
   ```mermaid
   flowchart LR
       I[Input Unit] --> C[CPU: ALU + Control Unit]
       C --> O[Output Unit]
       M[Memory Unit] <--> C
       S[(Storage)] <--> M
   ```
...
Answer: The question as printed is incomplete — a single number cannot give a resolution, because resolution is always expressed as **width × height**.
    ...
    (b) If 130 means pixels along ONE side
    - The resolution would be written as `130 × (the other dimension)`, and the second number must be given. For a square display it would be `130 × 130 = 16,900` pixels.

    - Standard answer to give: resolution is stated as width × height in pixels (such as 1920 × 1080), while a single number like 130 describes pixel density in PPI. The question needs both dimensions or the screen size before a numeric answer is possible.  <!-- verify -->
...
Answer: A computer system has five elements: hardware, software, data, people and procedures.
    ...
    (d) People (liveware)
    - Users, operators, programmers, system analysts and administrators. Without people the system has no purpose.

    (e) Procedures
    - The documented rules for operating the system: how to enter data, take backups, handle errors and recover from failure.

    ```mermaid
    flowchart LR
        I[Input Unit] --> C[CPU<br/>ALU + Control Unit]
        C --> O[Output Unit]
        M[Memory Unit] <--> C
    ```

    - All five must work together. Hardware without software does nothing, and software without trained people and clear procedures fails in practice.
...
Answer:
    ...
    | Point | BIOS (Legacy) | UEFI |
    |---|---|---|
    | Full form | Basic Input Output System | Unified Extensible Firmware Interface |
    | Operating mode | 16-bit — can address only about 1 MB of memory at boot | 32-bit or 64-bit — can use all system memory |
    | Boot speed | Slower, initialises devices one at a time | Faster, parallel initialisation |
    | Partition scheme | MBR — maximum 4 primary partitions | GPT — up to 128 partitions |
    | Maximum disk size | 2 TB | 9.4 ZB (effectively unlimited) |
    | Boot file location | Master Boot Record, sector 0 | An `.efi` file on the EFI System Partition |
    | Interface | Text only, keyboard driven | Graphical, mouse supported |
    | Security | None built in | Secure Boot — verifies digital signatures of the bootloader |
    | Network capability | None | Built-in network stack, can boot and update over network |
    | Modularity | Monolithic firmware | Modular, drivers can be added |

    Why UEFI replaced BIOS
    - The 2 TB disk limit of MBR became a hard blocker as drives grew.
    - Secure Boot prevents bootkits and rootkits from loading before the OS.
    - Faster boot matters on servers and laptops with SSDs.

    - Important practical note: switching between Legacy BIOS and UEFI mode makes an existing installation unbootable, because MBR and GPT partitioning are not interchangeable. The OS must be reinstalled or the disk converted.
...
Answer: A plotter is an **output device** — specifically a hard-copy output device.
    ...
    |---|---|---|
    | Output method | Draws continuous lines with a pen | Prints dots |
    | Best for | Line drawings, CAD, maps | Text and photographs |
    | Paper size | Very large, A0 and above | Usually A4 or A3 |
    | Speed | Slow | Fast |
...
Answer: BIOS is the PROGRAM; CMOS is the MEMORY that stores the BIOS settings. They are two different chips that work together.

    | Point | BIOS | CMOS |
    |---|---|---|
    | Full form | Basic Input Output System | Complementary Metal Oxide Semiconductor |
    | What it is | Firmware — a program | A memory chip |
    | Function | Runs POST, initialises hardware, loads the OS | Stores the BIOS configuration settings and the real-time clock |
    | Stored in | ROM / flash chip | A small volatile RAM chip |
    | Volatility | Non-volatile — survives power loss on its own | Volatile — needs constant power |
    | Power source | None needed | The CMOS coin-cell battery (CR2032) |
    | Contents | Boot code and I/O routines | Date, time, boot order, hardware settings, BIOS password |
    | Can it be changed | Only by flashing a firmware update | Changed every time the user saves BIOS setup |

    How they work together
    - BIOS is the code that runs; on startup it READS its configuration from CMOS.
    - When the user changes a setting in the BIOS setup screen and saves it, the value is written into CMOS.
    - When the CMOS battery dies, the settings and clock are lost, and the machine boots with factory defaults — but BIOS itself is unaffected, because it lives in non-volatile ROM.
...
Answer: A graphics card (also called a video card, display adapter or GPU card) is an expansion card that generates the image sent to the monitor. It takes over all graphics computation from the CPU.
    ...
    - Beyond graphics: because a GPU has thousands of cores, it is now the standard hardware for machine learning training, scientific simulation and cryptocurrency mining. This general-purpose use is called GPGPU.
...
Answer: Several hardware and software factors together decide how fast a computer runs.

    (a) CPU clock speed
    - Measured in GHz, it is the number of cycles per second. A 3.5 GHz processor executes more cycles than a 2.0 GHz one, all else being equal.

    (b) Number of cores and threads
    - A quad-core CPU can genuinely run four tasks at once. Multi-threading (Hyper-Threading) lets each core handle two threads, improving multitasking.

    (c) Cache memory
    - Small very fast memory (L1, L2, L3) between CPU and RAM. A larger cache means fewer trips to slow main memory. A cache miss can cost hundreds of CPU cycles.

    (d) RAM size and speed
    - If RAM is insufficient, the OS swaps pages to disk, which is thousands of times slower. This is the most common cause of a machine feeling slow. RAM speed (DDR4 vs DDR5) also matters.

    (e) Storage type
    - SSD versus HDD makes the largest visible difference on an older machine. An SSD has near-zero seek time and reads at 500 MB/s to 7 GB/s; an HDD manages about 100 MB/s with millisecond seek delays.

    (f) Bus speed and bandwidth
    - The system bus and PCIe generation limit how fast data moves between CPU, RAM and devices. A slow bus starves a fast CPU.

    (g) GPU
    - For graphics, video and machine learning work, the GPU rather than the CPU determines the speed.

    (h) Cooling and thermal throttling
    - An overheating CPU deliberately reduces its own clock speed to protect itself. A dusty heatsink or dried thermal paste can halve effective performance.

    (i) Software factors
    - Operating system efficiency, number of background processes and startup programs, malware, disk fragmentation on HDDs, and driver quality.

    - Practical priority for upgrading an old machine: SSD first, then more RAM, then CPU. The first two usually give far more improvement per taka than the third.
...
Answer:
    ...
    | Boot method | Reads the MBR at sector 0 | Runs an `.efi` file from the EFI System Partition |
    | Boot speed | Slower | Faster |
    | Interface | Text only | Graphical, mouse supported |
    | Security | None | Secure Boot with signature verification |
    | Network | None | Built-in network stack |

    (b) Firmware vs Bootloader

    | Point | Firmware | Bootloader |
    |---|---|---|
    | Definition | Permanent software embedded in hardware | A program that loads the operating system into memory |
    | Stored in | ROM or flash chip on the device | The boot sector or boot partition of a disk |
    | Runs when | Immediately on power-on, before anything else | After firmware, as the second stage |
    | Purpose | Initialise and control the hardware itself | Find, load and start the OS kernel |
...
Answer:
    ...
    | Point | OMR | MICR |
    |---|---|---|
    | Full form | Optical Mark Recognition | Magnetic Ink Character Recognition |
    | Technology used | Optical — reflected light detects dark marks | Magnetic — magnetised ink is read by a magnetic head |
    | What it reads | Filled marks or bubbles at fixed positions | Characters printed in special magnetic ink |
    | Ink required | Ordinary pencil or pen | Special magnetic ink containing iron oxide |
    | Character set | None — only mark position matters | A fixed font: E-13B (14 characters) or CMC-7 |
    | Accuracy | Very high | Extremely high, close to 100% |
    | Effect of dirt or folding | Can cause misreading | Reads correctly even through stamps, folds and signatures |
    | Security | Low — a mark is easy to forge | High — magnetic ink is hard to counterfeit |
    | Speed | Very fast | Very fast |
    | Main use | MCQ answer sheets, ballot papers, surveys, attendance | Bank cheque processing |

    Why banks chose MICR
