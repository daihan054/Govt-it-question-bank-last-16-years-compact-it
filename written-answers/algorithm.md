...
Answer: The question is `incomplete` — the paper records that the exact statements could not be collected. The true/false statements typically used about DFS and directed graphs are given below with brief reasons.

    ```
     1. DFS uses a STACK (explicit or recursion call stack).            TRUE
     2. DFS always finds the SHORTEST path in an unweighted graph.      FALSE
        (that is BFS; DFS finds A path, not the shortest)
     3. Time complexity of DFS is O(V+E) with an adjacency list.        TRUE
     4. Space complexity of DFS is O(V).                                TRUE
     5. DFS visits every vertex exactly once (with a visited array).    TRUE
     6. One DFS call visits every vertex of a disconnected graph.       FALSE
        (only its own component; outer loop must restart DFS on
         each unvisited vertex)
     7. A directed graph has a CYCLE iff DFS finds a BACK EDGE.         TRUE
     8. DFS on a directed graph classifies edges into 4 kinds:          TRUE
        tree (unvisited), back (ancestor -> cycle),
        forward (finished descendant), cross (other subtree)
     9. DFS on an undirected graph produces only tree and back edges.   TRUE
    10. Topological sort is possible for any directed graph.            FALSE
        (only for a DAG; a cycle makes it impossible)
    11. Topological sort = reverse of DFS finish order.                 TRUE
    12. Directed graph is STRONGLY CONNECTED if every vertex reaches
        every other (tested via Kosaraju's/Tarjan's, both DFS-based).   TRUE
    13. V vertices and V-1 edges implies a tree.                        FALSE
        (may be disconnected + cyclic, e.g. a 3-cycle + isolated vertex)
    14. Reversing every edge preserves strongly connected components.   TRUE
    ```
...
Answer: The question is `incomplete` — the algorithm whose complexity was to be analysed was cut off. The framework for stating time and space complexity, with standard results, is given below.

   ```
      TIME complexity  : how the number of basic OPERATIONS grows with n
      SPACE complexity : EXTRA (auxiliary) memory needed, beyond the input
   ```

   Asymptotic notations: `O` upper bound (worst case), `Ω` lower bound (best case), `Θ` tight bound.
   ...
   Sorting algorithms — common example
   ```
   ...
   ```
   - Searching: linear search O(n); binary search O(log n) on a sorted array; hashing O(1) average.

   How to derive a complexity
   ```
      1. Count the BASIC OPERATIONS as a function of n.
      2. Drop constants and lower-order terms: 3n^2+5n+100 -> O(n^2)
      3. Nested loops MULTIPLY; sequential loops ADD (keep the larger).
      4. For a recurrence use the MASTER THEOREM, e.g.
         Merge sort: T(n) = 2T(n/2) + O(n) -> O(n log n)
   ```
   - Space-time trade-off: `hashing` gives O(1) lookup at O(n) space; `dynamic programming` beats plain recursion by storing sub-results.
...
Answer: The question is `incomplete` — the paper records that the full question could not be collected. It concerns operations of complexity `O(N^2)`, covered below.

    What it means: `Quadratic` growth — doubling the input makes the work four times greater. Practical limit is roughly N = 10,000.

    Where it comes from — nested loops, each running N times:
    ```java
       for (i = 0; i < n; i++)
           for (j = 0; j < n; j++)
               // O(1) work                          ->  O(n^2)
    ```
    A triangular nested loop (`for j = i to n`) is still O(n^2) — half the work, same order, since constants are dropped.

    Common O(N^2) cases
    ```
       Bubble / Selection sort         : O(n^2) always
       Insertion sort                  : O(n^2) worst, O(n) nearly sorted
       Quick sort                      : O(n^2) worst case (bad pivot)
       Matrix add / transpose (n x n)  : O(n^2)
       BFS/DFS/Dijkstra with adjacency MATRIX : O(V^2)
       Comparing all pairs of elements : O(n^2) pairs
    ```

    How it is usually improved
    ```
       Problem              O(n^2) way          Better way
       Sorting               Bubble/selection    Merge/heap sort : O(n log n)
    ...
    ```
    - Trade-off is `space for time`: a hash set costs O(n) extra memory but cuts duplicate-checking from O(n²) to O(n).
    - Examiners look for: O(n²) is fine for small n but unacceptable for large n; standard cures are sorting first, hashing, DP, or a better data structure.
...
Answer: The question is `incomplete` — the exact weights/values/capacity were not collected. The comparison and calculation method are shown below with a worked example.

   ```
      FRACTIONAL knapsack : item may be broken; greedy by value/weight
           ratio is PROVABLY OPTIMAL — greedy cost = optimal cost, diff = 0.
      0/1 knapsack        : item taken WHOLE or not at all; greedy is
           NOT optimal — only DP gives the optimal answer.
   ```

   Worked example — Capacity W = 10
   ```
   ...
   0/1 knapsack, greedy by ratio: sort B(8.3), C(8.0), A(6.0).
   Take B (wt 6, val 50), remaining cap 4 — C(wt 5) and A(wt 10) both too big.
   **Greedy value = 50**, 4 units of capacity wasted.

   0/1 knapsack, optimal (DP/enumeration): best feasible subset is {A} alone (wt 10) = value 60.
   **Optimal value = 60**.
   ```
      LOSS = 60 - 50 = 10   (16.67% below optimal)
   ```
   Greedy picked B for its ratio, but that left an unusable capacity gap; DP finds the fit that greedy misses.

   Same items, FRACTIONAL knapsack: take all of B (wt 6, val 50) + 4/5 of C (wt 4, val 32) = **82**, which is the true optimum — difference = 0, because leftover capacity can always be filled with a fraction of the next-best item (greedy-choice property holds).

   Complexity: fractional greedy `O(n log n)`; 0/1 DP `O(nW)` time & space (pseudo-polynomial); 0/1 greedy `O(n log n)` but not optimal.
   - Lesson: greedy is fast but correct only when the problem has the greedy-choice property + optimal substructure (fractional knapsack, Huffman, Dijkstra, Prim, Kruskal); `0/1 knapsack` lacks it, hence needs DP.
...
Answer: Huffman coding is a greedy lossless compression method giving short codes to frequent characters and longer codes to rare ones; every code is a `prefix code` (unambiguous to decode).
   ...
   Final answer
   - Total bits with Huffman coding = `4+3+3+6+3+6 = 25 bits`; fixed-length (3 bits x 6 chars x ... ) needs `10 × 3 = 30 bits` → saving `5 bits` (~17%).
   - Encoded "CONNECTION" = `111 00 10 10 010 111 011 110 00 10`
   - Time complexity `O(n log n)` using a min-heap priority queue (`n` = distinct characters).
